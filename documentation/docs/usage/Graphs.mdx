...
* [Inject functional components](/docs/usage/FunctionalComponents)
* [Inject components](/docs/usage/ClassComponents)
* [Inject classes](/docs/usage/Classes)
...
### The singleton graph
Applications typically have at least one singleton graph. These graphs are used to provide dependencies that are used throughout the application. These dependencies are usually singletons, which means they should only be constructed once. The `ApplicationGraph` in the [example above](/docs/usage/Graphs#specifying-relationships-between-dependencies) is a singleton graph.
