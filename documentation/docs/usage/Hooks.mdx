...
type Props = DependenciesOf<AppGraph, 'fooService' | 'barService'>; // {fooService: FooService, barService: BarService}

export injectHook(myHook, AppGraph);
```
...
type Injected = DependenciesOf<AppGraph, 'fooService'>; // {fooService: FooService}
...
export default injectHookWithArguments<Injected, Own>(myHook, AppGraph);
```
...
type Injected = DependenciesOf<AppGraph, 'fooService'>;
type Own = {count: number};
...
export default injectHookWithArguments<Injected, Own, Result>(myHook, AppGraph);
```
