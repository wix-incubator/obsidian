...
## Wrapping up
While we didn't use any API from `Obsidian` in this refactor, this change was made possible by how Obsidian influences the design of our code. Obsidian makes it easy to introduce classes to each other by passing them explicitly in through the constructor. This approach encourages us to split our code into smaller classes that are easier to test.