...
tags: [MVVM, Architecture, Model, ViewModel, Reactivity, Graph, Memoization, Memoize]
---
...
:::tip Optimizing expensive computations
Models often contain methods that perform expensive calculations. You can use the `@memoize` decorator to cache results based on the method's arguments. The cached value is returned on subsequent calls with the same arguments, avoiding redundant computations. This is particularly useful when calling such methods from hooks during re-renders, and lets you avoid cluttering your view model with `useMemo` hooks.

```ts title="CounterModel.ts"
import { Observable, memoize } from "react-obsidian";
...
export class CounterModel {
  ...
  // Result is cached per unique multiplier value
  @memoize()
  public computeFactorial(n: number): number {
    if (n <= 1) return 1;
