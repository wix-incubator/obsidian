...
#### Inactive behavior
By default, lifecycle-bound graphs are cleared whenever React's `useEffect` cleanup fires. In some scenarios — such as React Native activity pauses or React StrictMode remounts — cleanup fires even though the component hasn't truly unmounted. This can cause the graph to be destroyed and recreated unnecessarily.

Setting `inactiveBehavior` to `'retain'` tells Obsidian to keep the graph alive during these transient cleanups and only clear it when the component is actually removed from the tree.

```ts title="A lifecycle-bound graph that survives activity pauses"
import {lifecycleBound, graph, ObjectGraph, provides} from 'react-obsidian';
...
:::note
`inactiveBehavior` can be combined with any `scope` value (`'feature'`, `'component'`, or custom).
:::
...
### Private subgraphs
Private subgraphs allow you to control the visibility of dependencies when composing graphs. Dependencies from private subgraphs can be used internally within the parent graph's provider methods, but are not exposed to external consumers of that graph.

This is useful when you want to encapsulate implementation details and prevent consumers from depending on internal infrastructure.

#### Use case
Consider an authentication graph that needs low-level infrastructure services like HTTP clients and configuration, but you don't want to expose these implementation details to consumers of the authentication graph.

<Tabs>
  <TabItem value="AuthGraph" label="AuthGraph">

```ts title="AuthGraph.ts"
import {singleton, graph, ObjectGraph, provides} from 'react-obsidian';
...
```
  ...
  <TabItem value="LoggingGraph" label="LoggingGraph">

```ts title="LoggingGraph.ts"
import {singleton, graph, ObjectGraph, provides} from 'react-obsidian';
...
```
  ...
  <TabItem value="InfrastructureGraph" label="InfrastructureGraph">

```ts title="InfrastructureGraph.ts"
import {singleton, graph, ObjectGraph, provides} from 'react-obsidian';
...
In this example:
- The `AuthGraph` can use both `httpClient` (from `InfrastructureGraph`) and `logger` (from `LoggingGraph`) in its provider methods
- Consumers of `AuthGraph` can access `logger` and `authService`, but **cannot** access `httpClient`

#### Visibility rules
- **Public subgraphs**: Dependencies are accessible both internally (within the graph's providers) and externally (to consumers via injection or `Obsidian.obtain()`)
- **Private subgraphs**: Dependencies are accessible only internally (within the graph's providers)
- **Transitive dependencies**: If a private subgraph has its own subgraphs, those transitive dependencies are also accessible internally
