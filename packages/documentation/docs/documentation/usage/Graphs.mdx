...
This is useful when you want to encapsulate implementation details and prevent consumers from depending on internal infrastructure that might change.
...
- This encapsulation ensures that consumers depend on the public API, not implementation details
...
#### Best practices
Use private subgraphs when:
- You want to hide implementation details from consumers
- You need infrastructure services that might change or be replaced
- You want to prevent coupling between consumer code and internal dependencies
- You're building a reusable module with a well-defined public API

Use public subgraphs when:
- Dependencies should be part of the graph's public API
- Consumers need direct access to the dependencies
- You want to compose multiple graphs that all expose their dependencies
