...
```bash
gh pr create --title "<title>" --body "<body>" && gh pr comment $(gh pr view --json number -q .number) --body "#skipreview #forcemerge"
```

**With force-merge** (user says "foce merge", "#forcemerge", etc.):
```bash
gh pr create --title "<title>" --body "<body>" && gh pr comment $(gh pr view --json number -q .number) --body "#skipreview #forcemerge"
```
